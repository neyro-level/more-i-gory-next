...
import { createFinalizeImportHandler, hashFeedBody } from "../src/core/ingest/finalize-import.ts";
import { createHash } from "node:crypto";

test("hashFeedBody is a stable sha256 of the fetched bytes", () => {
  const body = new TextEncoder().encode("<yml/>");
  assert.equal(hashFeedBody(body), createHash("sha256").update(body).digest("hex"));
  assert.equal(hashFeedBody(new Uint8Array()), null);
});
...
test("completed full run stamps lastSuccessfulRunAt and lastFullRunAt", async () => {
  const writes = [];
  const payload = {
    async findByID() {
      return { mode: "full" };
  ...
  const result = await handler({
    ...
    state: {
      fetch: { body, contentType: "application/xml", etag: '"n"', lastModified: "Wed", status: 200 },
      parse: { offeredCount: 2 },
  ...
  assert.equal(runWrite.data.feedHash, hashFeedBody(body));
