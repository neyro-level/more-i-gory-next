...
| `src/core/data-access/public/regions.ts:22` | `getPayload` | PUBLIC GATEWAY | `const payload = await getPayload({ config });` |
| `src/core/data-access/public/regions.ts:23` | `find` | PUBLIC GATEWAY | `const result = await payload.find({` |
