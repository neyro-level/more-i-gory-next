...
Обязательное framework-исключение: `src/app/(site)/error.tsx` является точечным
Client Component, потому что Next.js error boundary требует `"use client"` для
вызова `reset()`. Исключение ограничено этим файлом; детали ошибки пользователю
не выводятся, остальной site layout остаётся Server First.
