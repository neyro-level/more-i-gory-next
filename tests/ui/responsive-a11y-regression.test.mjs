...
const mobileNavigation = readFileSync("src/components/ui/mobile-navigation-sheet.tsx", "utf8");
const mobileNavigationPanel = readFileSync("src/components/ui/mobile-navigation-sheet-panel.tsx", "utf8");
const previewNavigation = readFileSync("src/components/ui/preview-navigation-menu.tsx", "utf8");
...
test("representative widths keep overflow, CTA, keyboard, labels and reduced motion contracts", () => {
  ...
  assert.match(mobileNavigation, /dynamic\(/);
  assert.match(mobileNavigation, /usePathname\(\)/);
  assert.match(mobileNavigationPanel, /<Sheet /);
  assert.match(mobileNavigationPanel, /aria-label="Мобильная навигация"/);
  assert.match(previewNavigation, /event\.key === "Escape"/);
