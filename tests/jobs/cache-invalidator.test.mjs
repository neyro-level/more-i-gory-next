...
test("CMS mutations map to CacheInvalidator targets after commit", () => {
  ...
  assert.ok(cmsMutationCacheTargets("regions", { pageKey: "CITY", parent: { slug: "krym" }, slug: "yalta" }).some((target) => target.kind === "path" && target.path === "/krym/yalta/"));
  assert.ok(cmsMutationCacheTargets("regions", { parent: { slug: "krym" }, slug: "novostroyki" }).some((target) => target.kind === "path" && target.path === "/investicionnaya-nedvizhimost/krym/novostroyki/"));
