...
const releaseScript = await readFile(new URL("../../scripts/ci/sourcecraft-release.sh", import.meta.url), "utf8");
const runtimeRelease = await readFile(new URL("../../scripts/verify-runtime-release.mjs", import.meta.url), "utf8");
...
test("one manual release workflow owns the single-build artifact path", () => {
  ...
  assert.ok(releaseScript.indexOf("pnpm install --frozen-lockfile") < releaseScript.indexOf("pnpm payload migrate"));
  assert.ok(releaseScript.indexOf("pnpm regions:seed") < releaseScript.indexOf("pnpm verify:risk:runtime-release"));
